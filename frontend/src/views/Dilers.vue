          ...
          <v-card-text>
            <v-container>
              <v-row>
                ...
                <v-col class="pt-0" cols="12">
                  <label>{{ $t('Diler Code') }}</label>
                  <v-text-field v-model="form.diler_code" dense></v-text-field>
                </v-col>
              </v-row>
            </v-container>
          </v-card-text>
...
export default {
  ...
  methods: {
    getVehicleList() {
      this.loading = true;
      this.$axios
        .post(this.$store.state.backend_url + '/api/getDiler', {
    ...
    deleteVehicle(item) {
      Swal.fire({
        ...
        if (result.value) {
          this.$axios
            .delete(
              this.$store.state.backend_url + '/api/dilers/delete/' + item.id
            )
            .then((res) => {
              console.log(res);
              this.getVehicleList();
    ...
    deleteFunction() {
      Swal.fire({
        ...
        if (result.value) {
          this.$axios
              .post(
                  this.$store.state.backend_url + '/api/dilers/delete-all'
              )
              .then((res) => {
                console.log(res);
                this.getVehicleList();
